...
The next question is do the mutants spend more time choosing a direction. So i first need to work out the direction they are travelling. This is called the heading, which can be stated in degrees. So I will first calculate the heading of the fish at each timepoint, then work out the change in heading with time. 

```{r}
# Calculate the heading in degrees 
# add it back to the data doc 
data %<>% 
  ungroup() %>% 
  mutate(
    direction.x = head.x - tailbase.x, 
    direction.y = head.y - tailbase.y, 
    heading.radians = atan2(direction.y, direction.x),
    heading.degrees = (heading.radians*180)/pi, 
    
    degrees.per.2second = heading.degrees/2,
    degrees.per.second = degrees.per.2second/2, 
    
    degrees.per.cm = heading.degrees/distance.cm
    
  )
```
```{r}
# plot out a couple of fish to check with this worked. 
data %>% 
  dplyr::filter(frame_idx < 500, fish_id %in% 2:10) %>% 
  ggplot() + 
   geom_segment(
     aes(xend = head.x, 
         yend = head.y, 
...
data %>% 
  ...
  ggplot(
    aes(x = head.x, y = head.y, colour = heading.degrees)
  ) +
  geom_point() + 
  facet_wrap(~fish_id)+ 
...
data %>% 
  ggplot(
    aes(x = frame_idx, y = heading.degrees, colour = geno_treat) 
  ) +
  geom_smooth(
...
data %>% 
  ...
  ggplot(
    aes(x = frame_idx, y = degrees.per.cm, colour = geno_treat)
  ) +
  # geom_point() + 
  geom_smooth(
    aes(group = geno_treat), 
