...
file_list <- list.files(path = here("data/exp1/raw_data/zones"),
                        pattern = "*.csv",
                        full.names = TRUE,
                        recursive = TRUE)
...
subject2fish[12,]$fish <- c("89 90 91 92 93 94 100 101")
...
# get rid of the trows which aresupposed to be col headers
df[[2]] %<>%
  lapply(function(x) {
    x %>%
...
write.csv(df, "data/exp1/processed_data/tidy_data.csv")
...
write.csv(df_a, "data/exp1/processed_data/time_in_zone.csv")
...
## STEP 7 - Final Output
write.csv(final_data, "data/exp1/processed_data/final_output.csv")
