...
file_list <- list.files(path = here("data/exp1.2/raw_data/zones"),
                        pattern = "*.csv",
                        full.names = TRUE,
                        recursive = TRUE)
...
df$file_list %<>%
  str_remove("/Users/karissabarthelson/Documents/2023_MPSIII_salbutamol/")
...
# give the last 2 fish dummy fish_ids so it doesn break the code later
subject2fish[12,]$fish <- c("89 90 91 92 93 94 1000 101")
...
# fix the first column here which does not match the rest
df[[3]][[12]] %<>%
  mutate(X1 = 0)
...
# get rid of the trows which are supposed to be col headers
df[[2]] %<>%
  lapply(function(x) {
    x %>%
...
# check all fish_ids are there
c(df$fish1, df$fish2, df$fish3, df$fish4, df$fish5, df$fish6, df$fish7, df$fish8) %>%
  unique %>%
  as.numeric() %>%
  sort

# see what cols have NA in them
df %>%
  dplyr::filter_all(any_vars(is.na(.))) %>%
  view

# omit these cols. these are the last rows which signifiy the final measurement
df %<>%
  dplyr::filter(arena %in% seq(1:8))
...
c(df$fish1, df$fish2, df$fish3, df$fish4, df$fish5, df$fish6, df$fish7, df$fish8) %>% unique %>% as.numeric() %>% sort
...
write.csv(df, "data/exp1.2/processed_data/tidy_data.csv")
...
df_a <- df_a %>%
  arrange(file_id,fish_id,elapsed_secs, exit_zone)  # order by fish_id and tie break by elepsed_secs
...
# get rid of my dummy fish
df_a %<>%
  dplyr::filter(fish_id != 101)

df_a %<>%
  dplyr::filter(fish_id != 1000)
...
write.csv(df_a, "data/exp1.2/processed_data/time_in_zone.csv")
...
final_data$fish_id %>% unique


## STEP 7 - Final Output
write.csv(final_data, "data/exp1.2/processed_data/final_output.csv")
